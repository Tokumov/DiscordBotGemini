import {
    Client, GatewayIntentBits, Partials, ButtonBuilder, ActionRowBuilder, ButtonStyle, ComponentType
} from "discord.js";
...
export function listenToDiscordBot() {
    ...
    client.on("messageCreate", async (message) => {
        if (greetings) { // Initial message is sent
            ...
            try {
                (await getOpportunitiesInfoFromText(message.content, 5)).forEach(opportunity => {
                    // let button;
                    // try {
                    ...
                    // }
                    // catch (e) {
                    //     console.log(opportunity.opportunityExtLink);
                    //     button = {
